...
interface Stock { fiche: number; numlist: number; maillist: number }
...
const TYPES = [
  ...
  },
  {
    ...
    cat: '02',
    stockKey: 'numlist' as keyof Stock,
  ...
  },
  {
    ...
    cat: '03',
    stockKey: 'maillist' as keyof Stock,
...
export default function DonneesSelection() {
  ...
  const { data: stock } = useQuery<Stock>({
    queryKey: ['donnees-stock'],
    queryFn: () => api.get('/api/donnees/stock').then((r) => r.data),
    staleTime: 30_000,
    retry: false,
  })
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', zIndex: 10, padding: '36px 0 16px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        }}>données digitales</div>
        <h1 style={{
          margin: 0, fontSize: 52, fontWeight: 900, lineHeight: 1,
          color: '#ffffff', fontFamily: '"Bebas Neue", "Impact", sans-serif', letterSpacing: '0.06em',
        }}>CHOISIR LE TYPE</h1>
        <div style={{ marginTop: 10, height: 1, width: 32, background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.2), transparent)' }} />
      </div>

      {/* 3 panels */}
      <div style={{ position: 'relative', zIndex: 10, flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', padding: '0 16px 20px', gap: 10 }}>
        {TYPES.map((t) => {
          const available = stock ? stock[t.stockKey] : null
          return (
            <button
              key={t.key}
            ...
            >
              ...
              {available !== null && (
                <div style={{
                  ...
                  <div style={{ fontFamily: '"Bebas Neue", sans-serif', fontSize: 20, color: t.accent, lineHeight: 1, letterSpacing: '0.04em' }}>
                    {fmt(available)}
                  </div>
                  <div style={{ fontSize: 7, fontFamily: '"JetBrains Mono", monospace', color: `${t.accent}66`, letterSpacing: '0.15em' }}>
                    DISPONIBLES
                  </div>
                </div>
              )}
              ...
              {/* Illustration */}
              <div style={{
                position: 'absolute', top: 14, right: available !== null ? 110 : 20,
                display: 'flex', flexDirection: 'column', gap: 3, opacity: 0.35,
                fontFamily: '"JetBrains Mono", monospace', fontSize: 8, color: t.accent, lineHeight: 1.6,
              }}>
              ...
              <div style={{ padding: '0 20px 18px', position: 'relative', zIndex: 2 }}>
                ...
                <div style={{
                  ...
                  fontFamily: '"Bebas Neue", "Impact", sans-serif', letterSpacing: '0.04em', marginBottom: 4,
                }}>{t.label}</div>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.38)', lineHeight: 1.4 }}>{t.sub}</div>
                <div style={{
      ...
      <style>{`
        ...
        @media (hover: hover) { .panel-btn:hover { transform: translateY(-2px); } }
      `}</style>
    </div>
  )
}
